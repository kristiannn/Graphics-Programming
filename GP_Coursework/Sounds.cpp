...
bool Sounds::BigEndian()
{
	int a = 1;
	return !((char*)&a)[0];
}

int Sounds::convertToInt(char* buffer, int length)
{
	int a = 0;
	if (!BigEndian())
...
char* Sounds::loadWAV(const char* fn, int& chan, int& samplerate, int& bps, int& size)
{
	char buffer[4];
...
unsigned int Sounds::loadSound(const char* filename)
{
	bool found = false;
...
void Sounds::deleteSound(unsigned int id)
{}

void Sounds::playSound(unsigned int id)
{
	alSourcePlay(id);
}
void Sounds::playSound(unsigned int id, glm::vec3& pos)
{
	alSource3f(id, AL_POSITION, pos.x, pos.y, pos.z);
	alSourcePlay(id);
}
void Sounds::stopSound(unsigned int id)
{
	alSourceStop(id);
}
void Sounds::setlistener(glm::vec3& pos, glm::vec3& camLookAt)
{
