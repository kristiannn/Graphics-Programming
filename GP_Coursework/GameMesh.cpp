...
void GameMesh::initialise(Vertex* vertices, unsigned int numVertices, unsigned int* indices, unsigned int numIndices)
{
	IndexedModel model;
...
void GameMesh::initialiseModel(const IndexedModel& model)
{

	drawCount = model.indices.size();

	glGenVertexArrays(1, &vertexArrayObj); //generate a vertex array object and store it in the VAO
...
void GameMesh::loadModel(const std::string& filename)
{
...
void GameMesh::updateSphere(glm::vec3 pos, float radius)
{
