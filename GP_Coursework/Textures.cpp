...
void Textures::initialise(const std::string& fileName)
{
	...
	glGenTextures(1, &textHandler); // number of and address of textures
	glBindTexture(GL_TEXTURE_2D, textHandler); //bind texture - define type 
...
Textures::~Textures()
{
	glDeleteTextures(1, &textHandler); // number of and address of textures
}
...
void Textures::Bind(unsigned int unit)
{
	...
	glBindTexture(GL_TEXTURE_2D, textHandler); //type of and texture to bind to unit
}

GLuint Textures::loadCubemap(std::vector<std::string> faces)
{
	glGenTextures(1, &textHandler);
	glBindTexture(GL_TEXTURE_CUBE_MAP, textHandler);
